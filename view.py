		...
		if 'cookie' in request.headers:
			...
			if ('gtoken' in cookie) and ('wsid' in cookie):
				...
				if mycams == user_data['cams']:
					...
					self.sendClose()
			elif ('wsid' in cookie) and ('PSClient' in request.headers['user-agent']):
				wsid = cookie['wsid'].value
...
class PSFactory(WebSocketServerFactory):
	...
	def stopProducing(self,client):
		if client.producer:
			client.producer.stopProducing
			
	def register(self,client):
		if client not in self.clients:
			print("added {0}".format(client.peer))
			self.clients.append(client)

	def unregister(self,client):
		if client in self.clients:
			print("removed {0}".format(client.peer))
