...
class WebMStream(object):
	...
	def make_pipeline(self):
		...
		self.appsource.set_property("format",Gst.Format.TIME)
		self.hlssink = self.pipeline.get_by_name("sink")
		self.hlssink.set_property("playlist-location","/tmp/"+self.camid+"/playlist.m3u8")
		self.hlssink.set_property("playlist-root","/tmp/"+self.camid)
		#print self.hlssink.get_property("target-duration")
		self.hlssink.set_property("target-duration",1)
		self.hlssink.set_property("playlist-length",10)
		self.hlssink.set_property("max-files",15)
		self.hlssink.set_property("location","/tmp/"+self.camid+"/segment%05d.ts")
		...
		if 'cookie' in request.headers:
			...
			if ('wsid' in cookie) and ('PSClient' in request.headers['user-agent']):
				...
				if cambot['id'] in request.path:
					temp_location = FilePath("/tmp/"+cambot['id'])
					if not temp_location.exists():
						temp_location.makedirs()
					return None
				else:
					self.sendClose()
			else:
